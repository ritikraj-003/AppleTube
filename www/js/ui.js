...
export class UIManager {
  ...
  static showShareModal(track) {
    if (!track) {
      UIManager.showToast('No song is currently playing', 'info');
      return;
    }
    ...
    // Build share data
    const songUrl = track.videoId
      ? `https://www.youtube.com/watch?v=${track.videoId}`
      : window.location.href;
    const shareText = `🎵 Listening to "${UIManager.escapeHtml(track.title)}" by ${UIManager.escapeHtml(track.artist)} on AppleTube`;
    const shareTitle = `${track.title || ''} — ${track.artist || ''}`;

    // Populate song preview card
    const thumb = document.getElementById('shareSongThumb');
    const title = document.getElementById('shareSongTitle');
    const artist = document.getElementById('shareSongArtist');
    const linkInput = document.getElementById('shareLinkInput');

    if (thumb)  thumb.src = track.thumbnail || 'assets/default-cover.svg';
    if (title)  title.textContent = track.title || 'Unknown Title';
    if (artist) artist.textContent = track.artist || 'Unknown Artist';
    if (linkInput) linkInput.value = songUrl;

    // Helper: replace listener cleanly
    const wire = (id, handler) => {
      const btn = document.getElementById(id);
      if (!btn) return;
      const old = btn._shareHandler;
    ...
    // WhatsApp
    wire('btnShareWhatsApp', () => {
      window.open(`https://wa.me/?text=${encodeURIComponent(shareText + '\n' + songUrl)}`, '_blank');
    });

    // Telegram
    wire('btnShareTelegram', () => {
      window.open(`https://t.me/share/url?url=${encodeURIComponent(songUrl)}&text=${encodeURIComponent(shareText)}`, '_blank');
    });

    // X / Twitter
    wire('btnShareTwitter', () => {
    ...
    wire('btnShareNative', async () => {
      if (navigator.share) {
        try {
          await navigator.share({ title: shareTitle, text: shareText, url: songUrl });
        } catch (err) {
          if (err.name !== 'AbortError') UIManager.showToast('Share failed', 'error');
        }
      } else {
        try {
          ...
          UIManager.showToast('Copy the link manually from the field above', 'info');
        }
      }
    });

    // Copy link button
    wire('btnCopyShareLink', async () => {
      const copyText = document.getElementById('btnCopyShareLinkText');
      try {
        await navigator.clipboard.writeText(songUrl);
        if (copyText) copyText.textContent = 'Copied!';
        UIManager.showToast('Link copied!', 'success');
    ...
    // Close button
    const btnClose = document.getElementById('btnCloseShareModal');
    if (btnClose && !btnClose._closeWired) {
      btnClose._closeWired = true;
      btnClose.addEventListener('click', () => shareModal.classList.remove('open'));
    }
