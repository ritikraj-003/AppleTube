...
class MusicAPI {
  ...
  // Returns the backend base URL — remote when deployed, '' when local
  getBase() {
  ...
  async checkLiveBackend() {
    ...
    try {
      const res = await fetch(`${this.getBase()}/api/status`, { signal: AbortSignal.timeout(1500) });
      if (res.ok) {
  ...
  async searchYouTube(query, limit = 30) {
    ...
    if (isLive) {
      try {
        const res = await fetch(`${this.getBase()}/api/yt/search?q=${encodeURIComponent(query)}&limit=${limit}`, {
  ...
  async getSearchSuggestions(query, limit = 8) {
    ...
    try {
      const res = await fetch(`${this.getBase()}/api/yt/suggestions?q=${encodeURIComponent(trimmed)}&limit=${limit}`, {
  ...
  async getYouTubeTrending() {
    ...
    if (isLive) {
      try {
        const res = await fetch(`${this.getBase()}/api/yt/trending`, { signal: AbortSignal.timeout(5000) });
        if (res.ok) {
          const items = await res.json();
          if (Array.isArray(items) && items.length > 0) {
            return items;
  ...
  async getRelatedTracks(track, overrideMood = null, limit = 20) {
    ...
    if (isLive) {
      try {
        ...
        const res = await fetch(`${this.getBase()}/api/yt/related?${queryParams.toString()}`, {
          signal: AbortSignal.timeout(6000)
        });
        if (res.ok) {
          const data = await res.json();
